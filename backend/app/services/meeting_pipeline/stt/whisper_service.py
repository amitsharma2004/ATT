...
class WhisperTranscriber:
    ...
    def transcribe(
        ...
        try:
            segments_iter, info = self._model.transcribe(
                ...
                vad_filter=True,
                vad_parameters=dict(
                    min_silence_duration_ms=500,  # don't split on brief mid-sentence pauses
                    speech_pad_ms=400,            # pad speech edges so soft starts/ends aren't clipped
                ),
                no_speech_threshold=0.4,          # lower = less likely to skip quiet/accented speech
                compression_ratio_threshold=2.4,  # detects looping; triggers temp fallback
                log_prob_threshold=-1.0,          # discard very low-confidence generations
                hallucination_silence_threshold=2.0,  # skip hallucinated text during silence
    ...
    @staticmethod
    def _deduplicate_internal_loops(text: str) -> str:
        """Detect and collapse cyclic sentence/clause repetitions generated by Whisper inside a single segment."""
        import re
        ...
        n = len(sentences)
        # Detect repeating pattern block of length k (from 1 to n//2)
        for k in range(1, n // 2 + 1):
            pattern = [s.lower() for s in sentences[:k]]
            repeats = 0
            idx = 0
            while idx + k <= n and [s.lower() for s in sentences[idx:idx+k]] == pattern:
                repeats += 1
                idx += k
            # If a block repeated 2+ times and covers majority of the text, collapse to first block
            if repeats >= 2 and idx >= n - k:
    ...
    def _collapse_repeats(
        ...
        """Collapse identical or near-identical segments that appear in close succession,
        and remove intra-segment repeating hallucinations.
        """
        if not segments:
            return []
        ...
        for seg in segments:
            # 1. First deduplicate internal sentence looping within this segment
            deduped_text = cls._deduplicate_internal_loops(seg.text)
            norm_curr = deduped_text.lower().strip()
            is_dup = False

            # 2. Compare against the last `max_lookback` segments
            lookback = cleaned[-max_lookback:]
            for idx, prev in enumerate(reversed(lookback)):
                norm_prev = prev.text.lower().strip()
                real_idx = len(cleaned) - 1 - idx

                # Exact match or fuzzy match for near-duplicates
                ratio = SequenceMatcher(None, norm_curr, norm_prev).ratio()
                if norm_curr == norm_prev or ratio >= similarity_threshold:
                    new_end = max(prev.end, seg.end)
                    cleaned[real_idx] = TranscriptionSegment(
                        start=prev.start,
                        end=new_end,
                        text=prev.text,
                    )
    ...
    def _process_segments(self, raw_segments: List[Any], info: Any) -> TranscriptionResult:
        ...
        # Deduplicate repeating / looping segments
        segments = self._collapse_repeats(segments)
